          ...
          return (
            <Link
              ...
              className={`group relative flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-all duration-200 ${
                active
                  ? "bg-gradient-to-r from-accent/15 to-transparent font-semibold text-accent shadow-[inset_2px_0_0_#f59e0b]"
            ...
            >
              ...
              {active && (
                <motion.span
                  ...
                  className="ml-auto h-1.5 w-1.5 rounded-full bg-accent shadow-[0_0_6px_#f59e0b]"
                />
              )}
            </Link>
          );
...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="fixed left-0 right-0 top-0 z-50 flex h-14 items-center justify-between border-b border-white/[0.06] bg-ink/95 px-5 backdrop-blur-md md:hidden">
      ...
      {/* Desktop sidebar */}
      <aside className="fixed left-0 top-0 hidden h-screen w-[260px] overflow-y-auto border-r border-white/[0.06] bg-ink-soft md:flex md:flex-col">
        <SidebarContent />
      </aside>
      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            <motion.div
              key="drawer"
              className="fixed left-0 top-0 z-50 h-full w-[260px] overflow-y-auto border-r border-white/[0.06] bg-ink-soft md:hidden"
              initial={{ x: "-100%" }}
              animate={{ x: 0 }}
              exit={{ x: "-100%" }}
